...
class TestPdf(TransactionCase):
    ...
    def test_ecommerce_pdf_file_reader(self):
    ...
    def test_ecommerce_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)

        pdf_writer = pdf.ecommercePdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
    ...
    def test_ecommerce_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.ecommercePdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.ecommercePdfFileReader(reader_buffer)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.ecommercePdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'ecommerce')
        self.assertEqual(pdf_info['/Creator'], 'ecommerce')
        reader_buffer.close()
