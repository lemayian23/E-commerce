...
import ecommerce
from ecommerce.http import Session
from ecommerce.addons.base.tests.common import HttpCaseWithUserDemo
from ecommerce.tools.func import lazy_property
from ecommerce.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.addClassCleanup(lazy_property.reset_all, ecommerce.http.root)
        cls.classPatch(ecommerce.conf, 'server_wide_modules', ['base', 'web', 'test_http'])
        lazy_property.reset_all(ecommerce.http.root)
        cls.classPatch(ecommerce.http.root, 'session_store', MemorySessionStore(session_class=Session))
        cls.classPatch(ecommerce.http.root, 'geoip_resolver', MemoryGeoipResolver())

    def setUp(self):
        super().setUp()
        ecommerce.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('ecommerce.http.db_list') as db_list, \
             patch('ecommerce.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('ecommerce.http.db_list') as db_list, \
             patch('ecommerce.http.db_filter') as db_filter, \
             patch('ecommerce.http.Registry') as Registry:
