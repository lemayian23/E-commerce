...
class Neutralize(Command):
    ...
    def run(self, args):
        ...
        opt = ecommerce.tools.config.parse_config(args)
        ...
        try:
            with ecommerce.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = ecommerce.modules.neutralize.get_installed_modules(cursor)
                    queries = ecommerce.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    ecommerce.modules.neutralize.neutralize_database(cursor)
