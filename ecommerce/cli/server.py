...
__author__ = ecommerce.release.author
__version__ = ecommerce.release.version

# Also use the `ecommerce` logger for the main script.
_logger = logging.getLogger('ecommerce')
...
def report_configuration():
    ...
    """
    config = ecommerce.tools.config
    _logger.info("ecommerce version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', ecommerce.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = ecommerce.tools.config
    if not ecommerce.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = ecommerce.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            ecommerce.tools.trans_export(config["language"],
...
def import_translation():
    ...
    registry = ecommerce.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = ecommerce.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    ecommerce.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                ecommerce.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except ecommerce.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        ecommerce.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = ecommerce.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the ecommerce server (default command)"""
    def run(self, args):
        ecommerce.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)
