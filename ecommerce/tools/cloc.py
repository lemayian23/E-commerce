...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in ecommerce.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [ecommerce.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(ecommerce.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be ecommerce/studio unless it comes from an imported module install
        ...
        for f in env['ir.model.fields'].browse(data.keys()):
            self.book(
                data[f.id][0] or "ecommerce/studio",
    ...
    def count_database(self, database):
        registry = ecommerce.registry(config['db_name'])
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="ecommerce cloc", lines="Line", other="Other", code="Code")
        s += hr
